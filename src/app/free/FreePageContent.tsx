...
function EmailGate({ onUnlock }: { onUnlock: () => void }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      localStorage.setItem("agentawake_free_unlocked", "1");
      onUnlock();
    } catch {
  ...
  return (
    <div className="my-10 rounded-2xl border border-[var(--accent)]/30 bg-gradient-to-b from-[var(--accent)]/[0.06] to-[var(--surface)] p-8 text-center shadow-[0_0_40px_rgba(232,119,46,0.08)]">
      <div className="text-3xl mb-3">🔓</div>
      <h3 className="text-xl font-extrabold mb-2 tracking-tight">Unlock the Full Chapter — Free</h3>
      <p className="text-sm text-[var(--text-secondary)] mb-1 max-w-sm mx-auto">
        Enter your email to keep reading. You&apos;ll get the three-file fix, platform guides, and new chapters delivered to your inbox.
      </p>
      <p className="text-xs text-[var(--accent-light)] font-semibold mb-6">No credit card. No spam. Unsubscribe anytime.</p>
      <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
        ...
        <button
          type="submit"
          disabled={loading}
          className="rounded-xl bg-[var(--accent-muted)] px-6 py-3 text-sm font-bold text-white hover:bg-[var(--accent)] transition-all hover:-translate-y-0.5 shrink-0 disabled:opacity-60"
        >
          {loading ? "Sending…" : "Unlock Free →"}
        </button>
      </form>
      {error && <p className="text-xs text-red-400 mt-2">{error}</p>}
      <p className="text-[10px] text-[var(--text-tertiary)] mt-3">Joining 100+ builders already using the system</p>
    </div>
  );
}
...
function EmailCapture() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // silent fail — email gate already captured
    }
  }
  ...
  if (submitted) {
    return (
      <div className="rounded-2xl border border-[var(--accent)]/20 bg-gradient-to-b from-[var(--accent)]/[0.04] to-[var(--surface)] p-8 text-center">
        ...
        <p className="text-sm text-[var(--text-secondary)]">We&apos;ll send new chapters straight to your inbox.</p>
      </div>
    );
  }
...
export default function FreePageContent() {
  ...
  useEffect(() => {
    if (localStorage.getItem("agentawake_free_unlocked") === "1") {
      setUnlocked(true);
    }
  }, []);
  ...
  return (
    <div className="min-h-screen bg-[var(--background)] text-[var(--foreground)]">
      ...
      <article className="max-w-2xl mx-auto px-6 py-12">
        ...
        {/* Content — always-visible first ~30% */}
        <div className="prose-custom space-y-5 text-[0.95rem] leading-relaxed text-[var(--foreground)]/80 [&_h2]:text-xl [&_h2]:font-bold [&_h2]:text-[var(--foreground)] [&_h2]:mt-10 [&_h2]:mb-4 [&_strong]:text-[var(--foreground)]">
          ...
          {/* Email gate — shown if not unlocked */}
        ...
        {/* Email capture (bottom) — always visible */}
