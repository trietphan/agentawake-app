...
export default function LangchainImplementation() {
  return (
    <>
      <p className="text-lg leading-relaxed mb-6">
        LangChain / LangGraph can absolutely run a proper three-layer brain. The trick is not "use more prompts" — it's giving the agent a pantry (knowledge), countertop (daily notes), and recipe book (tacit knowledge) that survive context resets.
      </p>

      <Analogy>
        LangChain / LangGraph is like a high-end espresso machine: powerful, fast, and expensive enough to hurt if misconfigured. This chapter is your barista training so you get consistent shots instead of random brown water.
      </Analogy>

      <h2>What We'll Build</h2>
      <p>Combine thread-level checkpointers, Store API namespaces, and retriever tools so state survives restarts.</p>
      ...
      <div className="my-6 grid gap-4 sm:grid-cols-3">
        <div className="rounded-xl border border-[var(--border)] bg-[var(--surface-hover)]/40 p-4">
          ...
          <p className="text-sm text-[var(--text-secondary)]">Retriever-backed vector store from PARA documents.</p>
        </div>
        <div className="rounded-xl border border-[var(--border)] bg-[var(--surface-hover)]/40 p-4">
          <div className="text-xs font-bold text-[var(--accent-light)] mb-1">Layer 2</div>
          <p className="text-sm text-[var(--text-secondary)]">Tool-based file/db logging per day + checkpoint snapshots.</p>
        </div>
        <div className="rounded-xl border border-[var(--border)] bg-[var(--surface-hover)]/40 p-4">
          <div className="text-xs font-bold text-purple-300 mb-1">Layer 3</div>
          <p className="text-sm text-[var(--text-secondary)]">Store API namespaces for preferences and patterns.</p>
        </div>
      </div>
...
python -m pip install langgraph langchain langchain-openai
`}</Code>
...
`}</Code>
      ...
      <Code title="platform-config">{`from langgraph.checkpoint.postgres import PostgresSaver
from langgraph.store.memory import InMemoryStore
store = InMemoryStore()
store.put(("tacit", "communication"), "style", {"value": "concise"})
`}</Code>
...
`}</Code>
      ...
      <p>LangGraph is flexible but complex; start with one graph + one checkpointer + one store namespace.</p>

      <Callout emoji="🔗" title="LangChain / LangGraph Implementation Guide: Quick Win">
...
`}</Code>
      ...
      <p>
        If you're starting from zero on LangChain / LangGraph, follow this exact timer-based sequence. Don't optimize yet — just establish a working baseline you can trust.
      </p>
      ...
      <h2>Architecture Diagram (LangChain / LangGraph)</h2>
   ...
   +--> Prune stale items + update change log`}</Code>
      ...
      <ol className="my-5 space-y-3 text-sm text-[var(--foreground)]/80 list-decimal pl-5">
        <li>
          ...
          <Code title="langgraph.config.ts">{`export default {
  checkpointer: {
    provider: "postgres",
  ...
  tracing: {
    ...
    project: "agentawake-memory"
        ...
        <li>
          ...
          <Code title="scripts/langgraph-cron.ts">{`import { graph } from "../src/agent/graph";
...
runDaily().catch(console.error);`}</Code>
        </li>
      ...
      <p>
        Coming from <strong>CrewAI task orchestration</strong>? The biggest shift in LangChain / LangGraph is operational discipline: explicit memory IO, explicit scheduling, explicit safety rules. Assume nothing is implicit.
      </p>
      <ul className="my-4 space-y-2 text-sm text-[var(--foreground)]/80">
...
- [ ] Restore test from latest backup`}</Code>
  ...
  "targets": {
    ...
    "time_to_recover_minutes": "< 15"
      ...
      <p>
        That's the full pattern for LangChain / LangGraph. Same brain architecture, different plumbing. Once this is stable, your agent stops being a clever intern and starts acting like an operator.
      </p>
    </>
  );
}
