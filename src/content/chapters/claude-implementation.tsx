...
export default function ClaudeImplementation() {
  return (
    <>
      <p className="text-lg leading-relaxed mb-6">
        Claude (Projects + Code CLI + MCP) can absolutely run a proper three-layer brain. The trick is not "use more prompts" — it's giving the agent a pantry (knowledge), countertop (daily notes), and recipe book (tacit knowledge) that survive context resets.
      </p>

      <Analogy>
        Claude (Projects + Code CLI + MCP) is like a high-end espresso machine: powerful, fast, and expensive enough to hurt if misconfigured. This chapter is your barista training so you get consistent shots instead of random brown water.
      </Analogy>

      <h2>What We'll Build</h2>
      <p>We combine Claude Projects for durable reference context, Claude Code for filesystem-native memory, and MCP for semantic recall when markdown is not enough.</p>
      ...
      <div className="my-6 grid gap-4 sm:grid-cols-3">
        <div className="rounded-xl border border-[var(--border)] bg-[var(--surface-hover)]/40 p-4">
          ...
          <p className="text-sm text-[var(--text-secondary)]">Claude Project files + PARA markdown + optional MCP index.</p>
        </div>
        <div className="rounded-xl border border-[var(--border)] bg-[var(--surface-hover)]/40 p-4">
          <div className="text-xs font-bold text-[var(--accent-light)] mb-1">Layer 2</div>
          <p className="text-sm text-[var(--text-secondary)]">memory/YYYY-MM-DD.md + Claude Code auto-memory notes.</p>
        </div>
        <div className="rounded-xl border border-[var(--border)] bg-[var(--surface-hover)]/40 p-4">
          <div className="text-xs font-bold text-purple-300 mb-1">Layer 3</div>
          <p className="text-sm text-[var(--text-secondary)]">.claude rules + user-level CLAUDE.md + tacit.md.</p>
        </div>
      </div>
...
mkdir -p .claude/rules knowledge/{projects,areas,resources,archives} memory
cat > .claude/CLAUDE.md <<'EOF'
# Project Memory Policy
- Read knowledge/tacit.md at session start
- Read memory/$(date +%F).md and previous day
- Update daily note after each major task
- Consolidate nightly into knowledge/
EOF
`}</Code>
...
`}</Code>
      ...
      <Code title="platform-config">{`{
  "mcpServers": {
    "memory": {
      "command": "npx",
      "args": ["-y", "mcp-memory-service"],
      "env": { "MEMORY_DIR": "./memory-store" }
    }
  }
}
0 2 * * * cd /workspace && claude --print "Consolidate daily notes into PARA docs"
`}</Code>
...
`}</Code>
      ...
      <p>Claude Projects are not API-first. For full automation, run Claude Code with scheduler and keep Projects as curated references.</p>

      <Callout emoji="🟣" title="Claude Implementation Guide: Quick Win">
...
`}</Code>
      ...
      <p>
        If you're starting from zero on Claude, follow this exact timer-based sequence. Don't optimize yet — just establish a working baseline you can trust.
      </p>
      ...
      <h2>Architecture Diagram (Claude)</h2>
  ...
  "permissions": {
    "allow": ["Read", "Write", "Bash(npm run build)"]
        ...
        <li>
          ...
          <Code title="scripts/claude-daily-sync.sh">{`#!/usr/bin/env bash
set -euo pipefail
...
claude --print "Read memory/\${today}.md and summarize top 3 decisions + blockers. Append summary to knowledge/projects/current-status.md"   >> logs/claude-sync.log 2>&1

echo "Daily sync complete: \${today}"`}</Code>
        </li>
      ...
      <p>
        Coming from <strong>ChatGPT / OpenAI Assistants</strong>? The biggest shift in Claude is operational discipline: explicit memory IO, explicit scheduling, explicit safety rules. Assume nothing is implicit.
      </p>
      <ul className="my-4 space-y-2 text-sm text-[var(--foreground)]/80">
...
- [ ] Restore test from latest backup`}</Code>
  ...
  "targets": {
    ...
    "time_to_recover_minutes": "< 15"
      ...
      <p>
        That's the full pattern for Claude (Projects + Code CLI + MCP). Same brain architecture, different plumbing. Once this is stable, your agent stops being a clever intern and starts acting like an operator.
      </p>
    </>
  );
}
