...
export default function ChatgptImplementation() {
  return (
    <>
      ...
      <Analogy>
        ChatGPT / OpenAI (Custom GPTs + Assistants API) is like a high-end espresso machine: powerful, fast, and expensive enough to hurt if misconfigured. This chapter is your barista training so you get consistent shots instead of random brown water.
      </Analogy>

      <h2>What We'll Build</h2>
      <p>Use Custom GPTs for interactive operations and Assistants API for persistence, threading, and automation through external schedulers.</p>
      ...
      <div className="my-6 grid gap-4 sm:grid-cols-3">
        <div className="rounded-xl border border-[var(--border)] bg-[var(--surface-hover)]/40 p-4">
          ...
          <p className="text-sm text-[var(--text-secondary)]">Vector Store with PARA files and metadata tags.</p>
        </div>
        <div className="rounded-xl border border-[var(--border)] bg-[var(--surface-hover)]/40 p-4">
          <div className="text-xs font-bold text-[var(--accent-light)] mb-1">Layer 2</div>
          <p className="text-sm text-[var(--text-secondary)]">External daily_notes table (Notion/Supabase/Postgres).</p>
        </div>
        <div className="rounded-xl border border-[var(--border)] bg-[var(--surface-hover)]/40 p-4">
          <div className="text-xs font-bold text-purple-300 mb-1">Layer 3</div>
          <p className="text-sm text-[var(--text-secondary)]">Assistant instructions + tacit document.</p>
        </div>
      </div>
...
`}</Code>
      ...
      <Code title="platform-config">{`from openai import OpenAI
client = OpenAI()
vector_store = client.vector_stores.create(name="agentawake-kb")
assistant = client.assistants.create(
  name="AgentAwake Ops",
  model="gpt-4.1",
  instructions="Use 3-layer memory and call daily-note tool"
)
`}</Code>
...
`}</Code>
      ...
      <p>Native ChatGPT memory is global and opaque; treat it as convenience, not source-of-truth.</p>

      <Callout emoji="🟢" title="ChatGPT / OpenAI Implementation Guide: Quick Win">
...
`}</Code>
      ...
      <h2>Architecture Diagram (ChatGPT / OpenAI)</h2>
      <Code title="memory-architecture.txt">{`User/Trigger
   |
   v
[Agent Runtime: ChatGPT / OpenAI]
   |
   ...
   +--> Prune stale items + update change log`}</Code>
      ...
      <ol className="my-5 space-y-3 text-sm text-[var(--foreground)]/80 list-decimal pl-5">
        <li>
          ...
          <Code title="assistant-memory-config.json">{`{
  ...
  "memory_policy": {
    ...
    "tacit": "assistant_instructions"
        ...
        <li>
          ...
          <Code title="scripts/openai-nightly-consolidation.ts">{`import { OpenAI } from "openai";
...
run().catch(console.error);`}</Code>
      ...
      <p>
        Coming from <strong>Claude Projects or Claude Code</strong>? The biggest shift in ChatGPT / OpenAI is operational discipline: explicit memory IO, explicit scheduling, explicit safety rules. Assume nothing is implicit.
      </p>
      <ul className="my-4 space-y-2 text-sm text-[var(--foreground)]/80">
