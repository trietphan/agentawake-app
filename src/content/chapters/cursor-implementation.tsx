...
export default function CursorImplementation() {
  return (
    <>
      <p className="text-lg leading-relaxed mb-6">
        Cursor / Windsurf / Cline can absolutely run a proper three-layer brain. The trick is not "use more prompts" — it's giving the agent a pantry (knowledge), countertop (daily notes), and recipe book (tacit knowledge) that survive context resets.
      </p>

      <Analogy>
        Cursor / Windsurf / Cline is like a high-end espresso machine: powerful, fast, and expensive enough to hurt if misconfigured. This chapter is your barista training so you get consistent shots instead of random brown water.
      </Analogy>

      <h2>What We'll Build</h2>
      <p>Implement the Memory Bank pattern and modern rule files so coding agents remember architecture and avoid repeated mistakes.</p>
      ...
      <div className="my-6 grid gap-4 sm:grid-cols-3">
        <div className="rounded-xl border border-[var(--border)] bg-[var(--surface-hover)]/40 p-4">
          ...
          <p className="text-sm text-[var(--text-secondary)]">.memory/project-brief + tech-stack + architecture docs.</p>
        </div>
        <div className="rounded-xl border border-[var(--border)] bg-[var(--surface-hover)]/40 p-4">
          <div className="text-xs font-bold text-[var(--accent-light)] mb-1">Layer 2</div>
          <p className="text-sm text-[var(--text-secondary)]">.memory/active-context + progress log updated every session.</p>
        </div>
        <div className="rounded-xl border border-[var(--border)] bg-[var(--surface-hover)]/40 p-4">
          <div className="text-xs font-bold text-purple-300 mb-1">Layer 3</div>
          <p className="text-sm text-[var(--text-secondary)]">.cursor/rules + decisions/errors files.</p>
        </div>
      </div>
...
mkdir -p .cursor/rules .memory
cat > .cursor/rules/memory-bank.mdc <<'EOF'
---
rule_type: always
---
At session start, read .memory/project-brief.md and .memory/active-context.md.
EOF
`}</Code>
...
---
Use vitest and avoid brittle snapshots.
`}</Code>
...
`}</Code>
      ...
      <p>Editor-native memories are convenient but not portable; keep canonical memory in repo files.</p>

      <Callout emoji="💻" title="Cursor / Windsurf / Cline Implementation Guide: Quick Win">
...
`}</Code>
      ...
      <p>
        If you're starting from zero on Cursor / Windsurf / Cline, follow this exact timer-based sequence. Don't optimize yet — just establish a working baseline you can trust.
      </p>
      ...
      <h2>Architecture Diagram (Cursor / Windsurf / Cline)</h2>
      <Code title="memory-architecture.txt">{`User/Trigger
   |
   v
[Agent Runtime: Cursor / Windsurf / Cline]
   |
   ...
   +--> Prune stale items + update change log`}</Code>
      ...
      <ol className="my-5 space-y-3 text-sm text-[var(--foreground)]/80 list-decimal pl-5">
        <li>
          ...
          <Code title=".cursor/rules/memory-bank.mdc">{`---
rule_type: always
---
...
5. Record architectural choices in .memory/decisions.md.`}</Code>
        </li>
        <li>
          <strong>Create one daily runner script.</strong> This is the backbone for your heartbeat/nightly memory behavior.
          <Code title="scripts/new-session.sh">{`#!/usr/bin/env bash
set -euo pipefail
...
echo "Session context reset"`}</Code>
        </li>
      ...
      <p>
        Coming from <strong>OpenClaw or Claude Code project memory</strong>? The biggest shift in Cursor / Windsurf / Cline is operational discipline: explicit memory IO, explicit scheduling, explicit safety rules. Assume nothing is implicit.
      </p>
      <ul className="my-4 space-y-2 text-sm text-[var(--foreground)]/80">
...
- [ ] Restore test from latest backup`}</Code>
  ...
  "targets": {
    ...
    "time_to_recover_minutes": "< 15"
      ...
      <p>
        That's the full pattern for Cursor / Windsurf / Cline. Same brain architecture, different plumbing. Once this is stable, your agent stops being a clever intern and starts acting like an operator.
      </p>
    </>
  );
}
