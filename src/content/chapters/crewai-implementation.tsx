...
export default function CrewaiImplementation() {
  return (
    <>
      ...
      <p>Wire CrewAI unified Memory to explicit scopes mirroring the three-layer architecture and make orchestration deterministic.</p>
      ...
      <div className="my-6 grid gap-4 sm:grid-cols-3">
        <div className="rounded-xl border border-[var(--border)] bg-[var(--surface-hover)]/40 p-4">
          ...
          <p className="text-sm text-[var(--text-secondary)]">Memory scopes under /knowledge/projects|areas|resources.</p>
        </div>
        <div className="rounded-xl border border-[var(--border)] bg-[var(--surface-hover)]/40 p-4">
          <div className="text-xs font-bold text-[var(--accent-light)] mb-1">Layer 2</div>
          <p className="text-sm text-[var(--text-secondary)]">/daily/YYYY-MM-DD scoped memories + end-run summarizer.</p>
        </div>
        <div className="rounded-xl border border-[var(--border)] bg-[var(--surface-hover)]/40 p-4">
          <div className="text-xs font-bold text-purple-300 mb-1">Layer 3</div>
          <p className="text-sm text-[var(--text-secondary)]">Agent backstories + /tacit scopes.</p>
        </div>
      </div>
...
`}</Code>
      ...
      <Code title="platform-config">{`from crewai import Memory
...
`}</Code>
      ...
      <p>Scope inference is good but not magic — pin critical data to explicit scopes for deterministic retrieval.</p>

      <Callout emoji="🚀" title="CrewAI Implementation Guide: Quick Win">
...
`}</Code>
      ...
      <h2>Architecture Diagram (CrewAI)</h2>
   ...
   +--> Prune stale items + update change log`}</Code>
      ...
      <ol className="my-5 space-y-3 text-sm text-[var(--foreground)]/80 list-decimal pl-5">
        <li>
          ...
          <Code title="config/crew.yaml">{`crew:
  name: memory-ops
  process: sequential
  memory: true
  verbose: true
  ...
  persist_summary:
    ...
    agent: operator`}</Code>
        ...
        <li>
          ...
          <Code title="scripts/run-crew.sh">{`#!/usr/bin/env bash
set -euo pipefail

cd "$(dirname "$0")/.."
python -m crewai run --config config/crew.yaml --inputs '{"date":"'"$(date +%F)"'"}' `}</Code>
        </li>
      ...
      <p>
        Coming from <strong>LangChain graphs or n8n AI Agent nodes</strong>? The biggest shift in CrewAI is operational discipline: explicit memory IO, explicit scheduling, explicit safety rules. Assume nothing is implicit.
      </p>
      <ul className="my-4 space-y-2 text-sm text-[var(--foreground)]/80">
